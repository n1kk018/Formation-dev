...
public class SubProcessWithBoundaryEventTestApp {
	
	private static Logger logger = LoggerFactory.getLogger(SubProcessWithBoundaryEventTestApp.class);
	
	private static final String ACTIVITI_SPRING_CONFIG="spring/service-activiti-spring.xml";
	private static final String BPMN_PROCESS_FILE="bpmn/subProcessWithBoundaryEvent.bpmn";
	private static final String BPMN_PROCESS_NAME="subProcessWithBoundaryEvent"; //id défini (via properties) à l'intérieur du fichier bpmn
	...
	public static void main(String[] args) {
		try{
			...
			String bpmnDeploymentIdForCalledActivity = myActivitiProcessManager.deployProcessDefinition("bpmn/MyFirstProcess.bpmn");
			...
			Thread.sleep(2 * 1000 * 60);//2mn
				
			//déchargement de la définition du processus BPMN:
			myActivitiProcessManager.undeployProcessDefinition(bpmnDeploymentIdForCalledActivity);
